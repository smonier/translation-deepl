...
public class DeepLTranslatorServiceImpl implements DeepLTranslatorService, ManagedService {
    ...
    public void translate(String path, String srcLanguage, String destLanguage) {
        try {
            ...
            LOGGER.info("destDeepLLanguage: "+destDeepLLanguage+" - srcLocale: "+srcLocale+" - destLocale: "+destLocale+" destLanguage: "+destLanguage);
             ...
             //       entry.setValue(translator.translateText(entry.getValue(), srcLanguage, destLanguage).getText());
                ...
                final boolean result = JCRTemplate.getInstance().doExecuteWithSystemSessionAsUser(null, Constants.EDIT_WORKSPACE, destLocale, (JCRSessionWrapper session) -> {
                    ...
                    for (Map.Entry<String, String> entry : translations.entrySet()) {
                        ...
                        LOGGER.info("Entry key : "+entry.getKey()+" -- translated Value: "+entry.getValue());
                    }
                   // node.saveSession();
                   // session.save();
                    return true;
            ...
            LOGGER.error("InterruptedException: "+String.format("Translation from %s to %s done for %s", srcLanguage, destLanguage, path), ex);
            Thread.currentThread().interrupt();
        } catch (DeepLException | RepositoryException ex) {
            LOGGER.error("Impossible to translate content: "+String.format("Translation from %s to %s done for %s", srcLanguage, destLanguage, path), ex);
        }
    }
